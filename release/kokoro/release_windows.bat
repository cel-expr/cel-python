...
@REM echo --- Applying ANTLR VERSION Collision Fix ---
@REM for /f "tokens=*" %%i in ('bazel info output_base') do set "OUTPUT_BASE=%%i"
@REM set "OUTPUT_BASE=!OUTPUT_BASE:/=\!"
@REM echo Output Base: !OUTPUT_BASE!

@REM set "ANTLR_DIR=!OUTPUT_BASE!\external\antlr4-cpp-runtime+"
@REM if exist "!ANTLR_DIR!\VERSION" (
@REM     if not exist "!ANTLR_DIR!\VERSION.txt" (
@REM         echo Renaming !ANTLR_DIR!\VERSION to VERSION.txt
@REM         ren "!ANTLR_DIR!\VERSION" VERSION.txt
@REM     )
@REM )
@REM if exist "!ANTLR_DIR!\version" (
@REM     if not exist "!ANTLR_DIR!\version.txt" (
