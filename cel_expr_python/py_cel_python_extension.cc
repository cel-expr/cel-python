...
cel::CompilerLibrary PyCelPythonExtension::GetCompilerLibrary() {
  // Capturing `this` is safe because it is guaranteed to outlive
  // PyCelEnvInternal, which creates and destroys a transient CompilerBuilder -
  // the only consumer of the CompilerLibrary.
